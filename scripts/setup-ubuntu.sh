...
LOCAL_RERANKER_DEVICE="${LOCAL_RERANKER_DEVICE:-cpu}"
RERANKER_TOP_K="${RERANKER_TOP_K:-30}"
RERANKER_WEIGHT="${RERANKER_WEIGHT:-0.35}"
...
PDF_OCR_LANG="${PDF_OCR_LANG:-vie+eng+chi_sim}"
PDF_OCR_SCALE="${PDF_OCR_SCALE:-2.0}"
PDF_OCR_MAX_PAGES="${PDF_OCR_MAX_PAGES:-100}"
OCR_MIN_LINE_CONFIDENCE="${OCR_MIN_LINE_CONFIDENCE:-0.35}"
PADDLE_OCR_LANG="${PADDLE_OCR_LANG:-vi,ch}"
PADDLE_OCR_DEVICE="${PADDLE_OCR_DEVICE:-cpu}"
VIETOCR_DEVICE="${VIETOCR_DEVICE:-cpu}"
VIETOCR_CONFIG="${VIETOCR_CONFIG:-vgg_transformer}"
PAGEINDEX_MIN_PAGES="${PAGEINDEX_MIN_PAGES:-1}"
CHAT_CONTEXT_LIMIT="${CHAT_CONTEXT_LIMIT:-5}"
CHAT_CONTEXT_MAX_CHARS="${CHAT_CONTEXT_MAX_CHARS:-3500}"
CHAT_CHUNK_MAX_CHARS="${CHAT_CHUNK_MAX_CHARS:-900}"
CHAT_MIN_RELEVANCE_SCORE="${CHAT_MIN_RELEVANCE_SCORE:-0.35}"
CHAT_MIN_LEXICAL_SCORE="${CHAT_MIN_LEXICAL_SCORE:-0.08}"
...
ensure_env_file() {
  ...
  set_env_value OCR_MIN_LINE_CONFIDENCE "$OCR_MIN_LINE_CONFIDENCE"
  set_env_value PADDLE_OCR_LANG "$PADDLE_OCR_LANG"
  set_env_value PADDLE_OCR_DEVICE "$PADDLE_OCR_DEVICE"
  set_env_value VIETOCR_DEVICE "$VIETOCR_DEVICE"
  set_env_value VIETOCR_CONFIG "$VIETOCR_CONFIG"
  set_env_value PAGEINDEX_MIN_PAGES "$PAGEINDEX_MIN_PAGES"
  set_env_value CHAT_CONTEXT_LIMIT "$CHAT_CONTEXT_LIMIT"
  set_env_value CHAT_CONTEXT_MAX_CHARS "$CHAT_CONTEXT_MAX_CHARS"
  set_env_value CHAT_CHUNK_MAX_CHARS "$CHAT_CHUNK_MAX_CHARS"
  set_env_value CHAT_MIN_RELEVANCE_SCORE "$CHAT_MIN_RELEVANCE_SCORE"
  set_env_value CHAT_MIN_LEXICAL_SCORE "$CHAT_MIN_LEXICAL_SCORE"
  ...
  set_env_value LOCAL_RERANKER_MODEL "$LOCAL_RERANKER_MODEL"
  set_env_value LOCAL_RERANKER_DEVICE "$LOCAL_RERANKER_DEVICE"
  set_env_value RERANKER_TOP_K "$RERANKER_TOP_K"
  set_env_value RERANKER_WEIGHT "$RERANKER_WEIGHT"
...
configure_api_provider() {
  ...
  set_env_value OLLAMA_HOST_PORT "$OLLAMA_HOST_PORT"
